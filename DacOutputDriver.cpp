...
DACOutputDriver::DACOutputDriver(DAC_HandleTypeDef* hdac, TIM_HandleTypeDef* htim, uint32_t DAC_CHANNEL, uint32_t DAC_ALIGN, uint32_t f){
	dac_port = hdac;
	tim_num = htim;
...
void DACOutputDriver::update_DAC(uint32_t* wave_LUT, uint16_t LUT_size){
	...
	HAL_TIM_Base_Init(tim_num); // Start the timer
